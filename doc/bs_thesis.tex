...
(in point where code is omitted in \cref{functrace})
...
\emph{trace}\footnote{A \emph{trace} a one enter point - multiple exit points sequence of instructions.}. Differently from the function instrumentation, here it can happen 
that a more than one trace have a BBL on the same address so, 
a preliminary check is done to avoid creation of 
...
technically the logic BBL is the same but the \verb|bbl| object is another.
