...
\subsection{The \emph{longjmp} problem}

The ANSI \verb|C| standard library provides a couple of functions called 
...
Results like this one can be obtained with IHPP by rebuilding it after 
disabling all advanced checks in \verb|config.h|.

\subsubsection{A first solution}
...
the functions in target program \emph{must} follow the \emph{calling conventions} and routines with variable arguments must \emph{never} use \emph{long jumps}.
...
on the stack, decreases its value and then jumps to the function address; even if it seems very strange, there are situations in which this does not happen: these not so unusual situations (specially using the \verb|cl| compiler) will be shown later.

The requirement of the second condition is simply but its reason is not obvious:
the \emph{stack pointer} check does not work in situations like the one shown in \cref{longjmp2}.
