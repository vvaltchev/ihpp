...
\begin{lstlisting}[label=out2, caption={The output of IHPP run with selective tracing}]
K value: INFINITE
====================================================================
Thread ID: 4294967296
====================================================================
...
\begin{lstlisting}[label=out3, caption={the output of IHPP with the joinThreads option}]
--------------------------------------------------------------------
DUMP of K-SF
--------------------------------------------------------------------
...
\begin{lstlisting}[label=out4, caption={a full IHPP output}]
K value: 2
Functions count: 18
...
\begin{lstlisting}[language=C, 
	caption={prog2.c, a simple program that uses recursion}, label=prog2, frame=leftline]
...
\begin{lstlisting}[label=out5, caption={an example of $k$-SF \emph{without} the \texttt{unrollSingleRec} option}]
| __root__(),1
   | main(),1
      | foo(),1
         | recFunc(),6
...
\begin{lstlisting}[label=out6, caption={an example of $k$-SF \emph{with} the \texttt{unrollSingleRec} option}]
| __root__(),1
   | main(),1
      | foo(),1
         | recFunc(),1
...
\begin{lstlisting}[language=C, 
	caption={prog3.c, a program that uses complex recursions}, label=prog3, frame=leftline]
...
\begin{lstlisting}[label=out7, caption={IHPP partial output for \texttt{prog3}}]
| __root__(),1
   | main(),1
      | foo(),1
         | bar(),1
...
When run with the \verb|-intraMode| option, IHPP traces BBLs inside all functions or inside the chosen ones, like in the \emph{function} mode. Every BBL is identified in output by three elements: the function which belongs to, the line and the column of the first 
...
\noindent
Running IHPP with \verb|-intraMode -ksf -k 100 -funcs foo,p| will produce in output:

\begin{lstlisting}[label=out8, caption={IHPP partial output for \texttt{prog4}}]
...
\noindent
A few considerations about \cref{out8}:
\renewcommand{\labelitemi}{$-$}
...
observe a good match between them, for example, the loop in lines 16-17 
...
From a theoretically point of view, this is \emph{like as} IHPP builds a different $k$-SF
for \emph{each} function activation and then joins all them. 
Practically, this is achieved in a better way.
