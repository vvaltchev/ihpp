...
\usetikzlibrary{arrows,graphs,trees,fit,shapes}
...
\draw [dashed] (-6,0) -- (-0.8,0) node [at start, above, draw=none, rectangle] {level 0};
\draw [dashed] (-6,-3) -- (-3,-3) node [at start, above, draw=none, rectangle] {level k};
\draw [dashed] (-6,-6) -- (-1.3,-6) node [at start, above, draw=none, rectangle] {level 2k};
...
\subsection{An algorithm for building the $k$-SF}
...
bla bla bla djasld jsal djls adjlq q lqj qje lwqje lqje:
\begin{center}
$k$-CCF $=$ $inv_k$($k$-SF)
\end{center}
