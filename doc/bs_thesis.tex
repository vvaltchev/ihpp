...
IHPP is a profiler written in C++ \emph{technically} 
as a \emph{plug-in}\footnote{In the pin-specific slang,
plug-ins like IHPP are called \emph{pintools}.} for the tool 
\textbf{Intel Pin}\footnote{Official page: \url{http://www.pintool.org}}.
...
with an option (called \verb|joinThreads|) to join all $k$-SFs and produce a cumulative $k$-SF-$k$-CCF couple of the program execution.
This working mode should be very simple to understand since it has been already explained in chapter 3. 
\end{paragraph}
...
Despite of its name (\emph{intra}procedural profiler), IHPP has also a particular and unique working mode that deals with BBLs but in which the concept \emph{procedures} is totally removed. In this working mode, a program is composed only by BBLs: there are no functions, so no calls and no returns. IHPP builds a couple of $k$-SF-$k$-CCF for each thread as other modes, but it is like the program has only one big procedure. This mode can be useful, for example, when one wants to analyze an algorithm implemented in two or more functions without considering the caller-callee relations but focusing the attention on execution flow of the hole algorithm. In then case when only one function is instrumented, there is no difference between \emph{inter-} and \emph{intra-} procedural modes.
