...
its behavior in order to discover possible bottlenecks in software. A basic
...
\url{https://software.intel.com/content/www/us/en/develop/articles/pin-a-dynamic-binary-instrumentation-tool.html}}; this last one is the tool used for the IHPP
project.
...
\mbox{\url{http://splashcon.org/2012/}}} data structures like $k$-SF and $k$-CCF, at least a basic explanation of these ones is \emph{strictly} necessary in this paper.
...
As explained in \emph{k-Calling Context Profiling}~\cite{kccf}, building (and handling) a CCT for a relatively long running program is unpractical and often useless.
...
(in this example, that value of $k$ is $3$): greater values of $k$ have no effect on the output. Apart of these considerations, $k$-CCF has not been formally defined yet because its definition uses an operation called \emph{tree join}.
...
all possible values of $k$ so, it should exist a way to build a $k$-CCF from a CCT: in fact, this is true and can be obtained using a particular operation called $k$-inverse forest, formally defined below.\\
...
Like PIN itself, IHPP works both on Microsoft \textbf{Windows} and \textbf{Linux}-based systems but, output data sometimes is different because of compiler-specific details. Under Linux systems the compiler \verb|gcc| is supported, while under Windows
the \verb|cl| compiler included in \textbf{Visual Studio} is supported.
This means that both the IHPP and the target program have to be compiled with the same compiler under a specific platform. Under Windows systems, target programs built with \verb|gcc| portings or other compilers are not supported due to problems with debug information: IHPP fetches the debug information of a program through PIN which accepts only \verb|pdb| debug info format under Windows and only \verb|dwarf| format under Linux-based systems; nevertheless, the problematic difference between the two compilers is of
a different kind: the \emph{calling conventions} (ABI) for a given platform are not always respected in auxiliary compiler-generated routines. Because of this, a huge amount of work has been done in IHPP to overcome these problems,
...
\noindent
The \verb|traceObject()| function mentioned before, works with the base objects
...
public:
	...
	//code omitted [...]
	node<keyT> &operator=(const node<keyT>& n);
};
...
void intraModeBlockTrace(BasicBlock *bb, /* omitted */) {
		...
		if (!globalSharedContext->options.rollLoops) {
			...
			while (parent) {
				...
				if (parent->getKey() == bb->getKey()) {
					...
					treeTop=parent; treeBottom=nullptr;
					found=true;
					treeTop->incCounter();
					break;
				}
	...
	if (!intraCtx->rootKey) {
		...
		treeTop=nullptr; treeBottom=nullptr;
		traceObject(bb, intraCtx, treeTop, treeBottom);
