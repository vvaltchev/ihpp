...
slowdown\footnote{The \emph{slowdown} is a factor which indicates how many times a program or an algorithm is \emph{slower} than another one}
measured should be always compared to the \emph{empty analysis} slowdown.
This last performance indicator depends from the analyzed program type: 
a program that makes an intensive use of \emph{recursion} has a bigger \verb|funcMode| slowdown than a program which uses mainly loops for heavy tasks.
...
\begin{tabular}{l|l|l}
& insTracing OFF & insTracing ON\\
\hline
funcMode & 2.5x - 4.0x & 60x - 110x\\
intraMode & 15x - 26x & 70x - 120x\\
interMode & 13x - 24x & (unused)\\
\end{tabular}}
